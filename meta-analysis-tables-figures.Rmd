---
title: "Associations Between Cognitive Appraisals and Emotions: A Meta-Analytic Review (Tables and Figures)"
...
This code is an accompaniment to the following paper:
...
and a link to the R Shiny application is: https://desmond-ong.shinyapps.io/meta-analysis-code/


This file contains code to reproduce the tables and figures in the paper.
...
```{r calculate-descriptives, echo=T, eval=T, warning=F, message=F}
...
### Analyzing Geography, Language, and Contexts
```{r geography-and-contexts, echo=T, eval=T}
# data <- read.csv('meta-analysis_data.csv',header=T) %>% 
#   filter(included=="yes") %>%
#   droplevels() %>%
#   mutate(Appraisal_Raw = Appraisal)
...
print("Inferred language -- mainly English or non-English")
table(data_singlestudy$inferredLanguage)


print("Participant sample type")
table(data_singlestudy$sample)

print("Experiment Context")
table(data_singlestudy$context)

print("Experimental Study? (No == Correlational Study)")
table(data_singlestudy$exp)


print("Dissertation Study? (No == Journal Study)")
table(data_singlestudy$Dissertation)

print("Summary of sample size")
summary(data_singlestudy$N)
```

### Bibliographic analysis (and Figure S1)
```{r bibliographic-analysis, echo=T, eval=T, warning=F, message=F}
# table(data_unique_articles$Dissertation) ## 59 dissertations, 192 articles
...
These next chunks are to create the static tables and figures for the paper. Several of the next chunks that write to file are current set to `eval=F` (i.e., these chunks don't run). 

```{r make-static-tables-for-paper, warning=F, echo=T, eval=F}
...
#### Figure 2: Bubble Plot of Effect Sizes

```{r read-in-table-data-for-plot, echo=T, eval=T, fig.width=7, fig.height=4}
table_common = read.csv('table-meta-analysis-results-common-emotions-raw.csv', header=T) %>% 
...
table_common %>% ggplot() + 
  ...
  geom_rect(xmin = 0.25, xmax = 1.00, ymin=0, ymax = 12,
            alpha=0.2, fill="#EEEEEE") +
  geom_point(aes(y=Emotion, x=abs(r), size=k, color=k, shape=r_significant), alpha=0.8,
  ...
  scale_y_discrete(limits = rev(levels(table_common$Emotion))) +
  scale_shape_manual(values = c(16, 1), name="Significant") +
  scale_size_continuous(name="Number of effect sizes", breaks = c(1,10,20,30,40)) + 
  # scale_color_distiller(name="Number of effect sizes", palette = "YlOrBr", 
  #                       direction = 1,
  #                       aesthetics = c("colour", "fill"),
  #                       limits=c(-20, 50),
  #                       breaks=c(10,20,30,40)) +
  scale_color_viridis_c(begin = 0, end = 1,
                       direction = -1, na.value = "#f9f9f9",
                       #option = "plasma", 
                       breaks = c(1,10,20,30,40),
                       name = "Number of effect sizes") +
  guides(shape = guide_legend(nrow = 2, byrow = T),
         size = guide_legend(nrow = 5, byrow = T), 
         color = guide_legend(nrow = 5, byrow = T)) +
...
#### Figure 3: Heatmap (Number of studies for each appraisal/emotion relationship)

```{r make-static-figures-heatmap, message=F, warning=F, echo=T, eval=T, fig.width=12, fig.height=9}
## Set `eval=T` to run this chunk.

heatmap_df = data %>% 
      #group_by(Emotion, Appraisal) %>%
      group_by(Emotion, Appraisal, .drop = FALSE) %>% ## this line is to calculate k=0's
...
heatmap_df %>% 
  ...
  # scale_fill_distiller(name="Number of effect sizes", 
  #                      #palette = "Oranges",
  #                      palette = "YlOrBr", 
  #                      direction = 1,
  #                      na.value = "#f9f9f9",
  ...
  #                      #guide = guide_colorbar(draw.llim = FALSE)
  #                      ) +
...
### Table 3: Average effect sizes for different appraisal-emotion relationship types

```{r predicted-relationships-test, warning=F, echo=T, eval=T}
d2 <- read.csv("predicted_relationships.csv") %>% 
  rename(appraisal = Appraisal, emotion=Emotion) %>% 
  mutate(
...
## Testing if effect sizes in 'Predicted and Supported' are higher than in 'No Prediction and Significant': (note that we convert the pearson `r`'s to `z`'s and do a two-sample t-test.)
...
### Table 4: Profiles Table (Appraisals with larger relationships (r >.25) or smaller relationships)

```{r make-profiles-table, warning=F, message=F, echo=T, eval=T}
full_table = data.frame()
...
### Table S6: Moderator Analysis Table
```{r make-static-tables-for-paper-moderator, warning=F, echo=T, eval=F}
#### Moderator Analyses
...
# number relationships tested: nrow(full_table) = 73
# number relationships significant: nrow(full_table_significant_moderator) = 21
# of these, same sign are: nrow(full_table_significant_moderator_same_pattern) = 9
# that means 12 are different sign.

```


### Table S7: Table for References to predictions
```{r make-static-tables-for-paper-prediction, warning=F, echo=T, eval=F}
## this chunk is used to write the prediction tables
...
```{r heterogeniety-analyses, echo=F, eval=F}
pred <- read.csv('predicted_relationships.csv', header=T) %>% 
  arrange(Emotion, Appraisal) %>%
  mutate(I2 = NA)
for(thisRow in c(1:nrow(pred))) {
  this_emotion = pred$Emotion[thisRow]
  this_appraisal = pred$Appraisal[thisRow]
  df = data %>% filter(Emotion == this_emotion,
                         Appraisal == this_appraisal)
  if(nrow(df)>=1) {
    this_table = meta_analysis_for_table(df, this_emotion,
                                         pubBias=FALSE,
                                         interpretation=FALSE)
    pred$I2[thisRow] = this_table$I2[1]
  }
}

# Considering only k>3, (41 cases of "No prediction & Significant”, 68 cases of “Predicted & Supported”)
pred %>% filter(k>3) %>% mutate(I2 = as.numeric(I2)) %>% summarize(meanI2 = mean(I2, na.rm=T))
pred %>% filter(k>3,
                prediction %in% c("Predicted & Supported", "No prediction & Significant")) %>%
  mutate(I2 = as.numeric(I2)) %>%
  group_by(prediction) %>% 
  summarize(meanI2 = mean(I2, na.rm=T), sdI2 = sd(I2, na.rm=T), num = n())

pred %>% filter(k>3,
                prediction %in% c("Predicted & Supported", "No prediction & Significant")) %>%
  mutate(I2 = as.numeric(I2)) %>%
  t.test(I2 ~ prediction, data=.) 
